...
type ArcParams = [number, number, number?, number?, number?, number, number];
type CubicBezierCurve = [number, number, number, number, number, number];
...
const approximateUnitArc = (thisAngle: number, thatAngle: number): [Point, Point, Point] => {
    // http://spencermortensen.com/articles/bezier-circle
    const a = thatAngle === 1.5707963267948966 ? 0.551915024494 
            : thatAngle === -1.5707963267948966 ? -0.551915024494 
            : 4 / 3 * Math.tan(thatAngle / 4);
...
export const arcToCubicBeziers = (
    previousPoint: Point,
    [ellipseRadiusX, ellipseRadiusY, xAxisRotation = 0, largeArcFlag = 0, sweepFlag = 0, nextPointX, nextPointY]: ArcParams
): CubicBezierCurve[] => {
    ...
    let [centerX, centerY, thisAngle, thatAngle] = getArcCenter(
        previousPoint,
        nextPointX,
        nextPointY,
        ellipseRadiusX,
